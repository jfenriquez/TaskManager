...
export function registerCategoryTools(server: McpServer, userId: string) {
  server.registerTool(
    ...
    async () => {
      ...
      const text = categories.map((c) => `${c.name} (${c.color}) [${c.id}]`).join("\n");
      return {
        content: [
  ...
  server.registerTool(
    ...
    async (input) => {
      const category = await prisma.category.create({ data: { name: input.name, color: input.color, userId } });
  ...
  server.registerTool(
    "update_category",
    {
      ...
      inputSchema: z.object({ id: z.string(), name: z.string().min(1).optional(), color: z.string().optional() }),
    },
    async (input) => {
      const data: Record<string, string> = {};
      if (input.name !== undefined) data.name = input.name;
      if (input.color !== undefined) data.color = input.color;
      const category = await prisma.category.update({ where: { id: input.id, userId }, data });
      return { content: [{ type: "text", text: JSON.stringify(category, null, 2) }] };
    }
  );
  ...
  server.registerTool(
    ...
    async (input) => {
      await prisma.tasks.updateMany({ where: { categoryId: input.id, userId }, data: { categoryId: null } });
      const category = await prisma.category.delete({ where: { id: input.id, userId } });
      return { content: [{ type: "text", text: `Categoría "${category.name}" eliminada` }] };
    }
  );
}
