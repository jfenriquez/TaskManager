    ...
    "start_timer",
    {
      ...
      inputSchema: z.object({
        ...
        minutes: z
          ...
          .positive()
          .optional()
          .describe("Minutos para el timer (usa timerMinutes de la tarea si no se especifica)"),
      }),
    },
    async (input) => {
      ...
      if (!secondsToUse || secondsToUse <= 0) {
        throw new Error(
          "No hay duración de timer. Especifica 'minutes' o asigna timerMinutes a la tarea."
        );
      }
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      if (!task.timerRunning || !task.timerEndsAt) {
        return {
          content: [{ type: "text", text: "El timer no está corriendo" }],
        };
      }

      const remainingSec = Math.max(
        0,
        Math.ceil((task.timerEndsAt.getTime() - Date.now()) / 1000)
      );
