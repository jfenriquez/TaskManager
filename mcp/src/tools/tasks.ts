...
import { normalizeDateUtc, getDateRangeUtc } from "../utils/date.js";
...
export function registerTaskTools(server: McpServer) {
      ...
      const task = await prisma.tasks.create({
        data: {
          ...
          ExecutionDate: input.executionDate
            ? normalizeDateUtc(input.executionDate, tz)
            : null,
          userId,
        },
      });
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      if (input.executionDate !== undefined) {
        ...
        const tz = user?.timezone || "UTC";
        data.ExecutionDate = input.executionDate
          ? normalizeDateUtc(input.executionDate, tz)
          : null;
      }
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      return {
        content: [{ type: "text", text: `Tarea "${existing.title}" eliminada` }],
      };
    }
  );
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      // Actualizar racha y dailyLog (misma lógica que updateStatusTask)
      if (input.completed) {
    ...
    async (input) => {
      ...
      const tasks = input.tasks.map((t) => ({
        ...
        ExecutionDate: t.executionDate
          ? normalizeDateUtc(t.executionDate, tz)
          : null,
      ...
      const created = await prisma.$transaction(
        tasks.map((data) => prisma.tasks.create({ data }))
      );
      ...
      return {
        content: [
          {
            type: "text",
            text: `Importadas ${created.length} tareas correctamente`,
          },
        ],
      };
    }
