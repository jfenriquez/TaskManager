...
import { getDateRangeUtc } from "../utils/date.js";
...
export function registerResources(server: McpServer) {
  server.registerResource(
    ...
    async (uri, vars) => {
      ...
      const task = await prisma.tasks.findFirst({
        where: { id: vars.id as string, userId },
      });
      if (!task) {
        return {
          contents: [{ uri: uri.href, mimeType: "text/plain", text: "Task not found" }],
        };
      }
      return {
        contents: [
  ...
  server.registerResource(
    ...
    async (uri) => {
      ...
      return {
        contents: [
          { uri: uri.href, mimeType: "application/json", text: JSON.stringify(categories, null, 2) },
        ],
      };
    }
  );
        ...
        contents: [
          {
            uri: uri.href,
            mimeType: "application/json",
            text: JSON.stringify({ currentStreak: user?.streak ?? 0 }, null, 2),
          },
        ],
      };
    }
